...
# QUAN TRỌNG: Tăng giới hạn Payload để nhận chuỗi Base64 nén
...
def AI_image():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash("AI Engine đang bận. Vui lòng thử lại sau!")
            return redirect(url_for('phan_loai'))
...
def save_feedback():
    ...
    try:
        data = request.get_json(force=True) # Ép kiểu JSON để tránh lỗi Header
        feedback_entry = {
        ...
        print(f"DEBUG ERROR: {str(e)}") # Log lỗi ra console của Render
        return jsonify({"status": "error", "message": "Server không thể xử lý ảnh"}), 500
...
def signup():
    if request.method == 'POST':
        ...
        if not username or not email or not password:
            flash("Không được để trống!"); return redirect(url_for('signup'))
        if password != confirm:
        ...
        flash("Thành công! Mời đăng nhập.")
        return redirect(url_for('login'))
...
def login():
    if request.method == 'POST':
        ...
        password = request.form.get('password', '')
        user = mongo.db.users.find_one({"$or": [{"username": identity}, {"email": identity}]})
